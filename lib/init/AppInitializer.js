...
const AppInitializer = {
  ...
  init: async (app) => {
    ...
    for (const routeName of Object.keys(CONFIG.routes)) {
      logger.debug(`Обработка роута: ${routeName}`)
      ...
      logger.debug(`Сформирован путь для роутера ${routeName}: ${routePath}`)
      ...
      try {
        ...
        logger.info(`Роутер ${routeName} успешно подключен. Путь: ${routeConfig.path || '/routes'}, Метод: ${routeConfig.method || 'GET'}`)
        logger.debug(`Детали роутера ${routeName}: Путь - ${routeConfig.path || '/routes'}, Метод - ${routeConfig.method || 'GET'}`)
      } catch (error) {
    ...
    logger.debug('Инициализация роутов завершена.')
  }
}
