...
const checkKey = (route, value) => {
  // Преобразование типов данных и обработка невидимых символов
  const stringValue = String(value).trim()

  logger.debug(`Начало проверки ключа для маршрута: ${route}, Тип маршрута: ${typeof route}`)
  logger.info(`Проверяемый ключ: ${stringValue}, Тип ключа: ${typeof stringValue}`)
  ...
  if (clientKeys && Object.values(clientKeys).includes(stringValue)) {
    logger.info(`Доступ разрешен для клиента: ${route}`)
    return
  }
  ...
  if (controllerKeys && String(controllerKeys[route]).trim() === stringValue) {
    logger.info(`Доступ разрешен для контроллера: ${route}`)
    return
  }
